...
while ball[N-3].pos.y < 0.1*A:
    ...
    for i in range(1,N):      
        if i == N-1: 
            ...
            Ft = mag(SpringForce(spring[i-1].axis)-ft(spring[i-1].axis))

mu = (m*N)/((N-1)*d)

wave_v = sqrt(0.5*d*k/mu) #(___繩上張力___/___繩線密度___)

print ("theoretical value for mean wave velocity:", wave_v)
print ("simulated value for mean wave velocity:", (N-1)*d/t)
