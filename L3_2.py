...
ball30 = sphere(pos = vec(0, 0, 0), vc = vec(v * cos(30 * pi / 180), v * sin(30 * pi / 180), 0), radius = 0.5, color = color.blue, make_trail= True, trail_type = "curve", interval=1)
ball60 = sphere(pos = vec(0, 0, 0), vc = vec(v * cos(60 * pi / 180), v * sin(60 * pi / 180), 0), radius = 0.5, color = color.green, make_trail= True, trail_type = "curve", interval=1)

while ball30.pos.y >= 0 or ball60.pos.y >= 0:
    rate(1 / dt)
    if ball30.pos.y >= 0:
        ball30.vc -= k * ball30.vc * dt
        ball30.pos += ball30.vc * dt
        t30 += dt
    if ball60.pos.y >= 0:
        ball60.vc -= k * ball60.vc * dt
